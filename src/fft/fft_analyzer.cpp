...
namespace openspectrum
{
          ...
          m_freq_bins(fft_size / 2 + 1)
    {
        ...
        m_cfg = kiss_fft_alloc(static_cast<int>(fft_size), m_inverse ? 1 : 0, nullptr, nullptr);
        if (!m_cfg)
        {
                              ...
                              std::vector<std::complex<float>> &output)
    {
        ...
        if (output.size() >= m_fft_size)
        {
            for (size_t i = 0; i < half_size; ++i)
            {
                ...
                output[i] = std::complex<float>(m_output_buffer[out_idx].r, m_output_buffer[out_idx].i);
            }
        }
    }
