...
void FftAnalyzer::execute(const std::vector<std::complex<float>> &input,
  ...
  for (size_t i = 0; i < m_fft_size; ++i) {
    ...
    m_input_buffer[i] = pocketfft_cpx(sign * input[i].real(), sign * input[i].imag());
  }

  // Execute FFT using PocketFFT directly (no conversion needed)
  if (m_inverse) {
  ...
  for (size_t i = 0; i < half_size; ++i) {
    ...
    float const real = m_output_buffer[out_idx].real();
    float const imag = m_output_buffer[out_idx].imag();
  ...
  if (output.size() >= m_fft_size) {
    for (size_t i = 0; i < half_size; ++i) {
      ...
      output[i] = std::complex<float>(m_output_buffer[out_idx].real(),
                                      m_output_buffer[out_idx].imag());
    }
  }
}
