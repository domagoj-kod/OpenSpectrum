...
**The opt-in 32768/65536 sizes invert that** (i7-12700H / D3D11 / 2.048 MS/s): `cpu` = 0.80 / 1.55 / 3.25 ms at 16384 / 32768 / 65536 while `render_build` holds ~1.5 ms — so **65536 is the one configuration where the pipeline is DSP-bound**. That is ~4× the CPU of 16384 (2.4% → 9.8% of a core) buying RBW the display cannot show: ~1.4 kHz/px at this span vs 125 Hz already at 16384. Both still hold 30 fps — but **65536's headroom is ~4% and it is arithmetic, not performance**: the frame supply is `rate / fft_size` = **31.25/s** at 2.048 MS/s, barely over the 30 fps default cap, so raising `--max-fps` above 31 does nothing there. 131072 is excluded on purpose — 4 device callbacks per frame caps the line rate at ~15.6/s (measured 15.4–16.0), the cap `buf_len=65536` exists to escape; 65536 is just the last size clearing the default cap.

**WSL2 predicted these within 5%** (1.63 / 3.50) — but read that narrowly. It means *absolute* `cpu` on the **same CPU** across backends. It does **not** license extrapolating a *delta* to another machine: a WSL2/i7-12700H A/B of the plan cache predicted `cpu` −26% and the T470 delivered **+16%** — a different governor flipped the **sign**. `cpu` is still the cleanest cross-backend number and `render_build`/`present` are still not, but `cpu` is wall-clock, so it tracks the governor's clock as much as the work (see the `cpu` caveat in `docs/TECHNICAL.md`). Measure deltas on the target.
...
- **Software renderer** = battery/thermal worst case, but ~2.5× OpenGL, not a disaster — **the old "~16 ms" figure does not reproduce**. Measured T470 / Linux / 1050×576 / `--max-fps 30` / FFT 32768: `render_build`+`present` ≈ **5.9 ms** (3.17 + 2.77, vsync off so `present` is real blit work) vs ~2.3 ms on OpenGL. That gap is a flat **+3.6 ms/frame and does not grow with FFT size** — the scroll path decouples render cost from bin count. Still single-core, still warns loudly, still fallback-only; `--max-fps 30` helps most here. The ~16 ms predates this and its conditions are unknown; the v3.8.1 case study's software number (`render_build` 9.7 → 6.7 ms) is a *different machine* (i7-12700H/Windows), so don't reconcile the two — re-measure per box.
- **Memory is bounded, and scales mildly with FFT size:** ~51 MB resident at FFT 16384 on Direct3D 11 / Windows (measured at `99257e3`; the older "<49 MB" figure was v3.7.0). The frame pool is `MAIN_FRAME_POOL_FRAMES` (12) × `fft_size` × 8 B, so it grows with the transform — measured **+8 MB going 4096 → 65536**. Bounded, not free: an earlier "not FFT-scaling" claim was an artifact of 16384 being the largest size available. A Linux/WSL2 profile reads much higher — that's the Mesa software-GL backing store (no GPU passthrough), not heap. Full breakdown in `docs/TECHNICAL.md`.

### ControlState flow
**The 4096 default is deliberate — don't "optimize" it up.** Every benchmark in this repo runs 16384 because it was the *ceiling*, not because it is the target: the reasoning is that if a 15 W i5-7300U sustains the worst case, weaker hardware is safe. The default is sized the other way, for the machine we *don't* control — a radio engineer on a 6th-gen i3, not a modern desktop. 4096 costs ~0.15 ms cpu where 16384 costs ~0.80, and both look fine since the mean detector + averaging landed. Raising it trades that headroom for resolution the pane cannot show (see the autoscale section). If you change it, justify it against the oldest machine you expect to run this, not the fastest.
