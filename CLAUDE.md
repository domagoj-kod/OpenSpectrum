...
- **Measure on the target, with counters, in one session.** Deltas do **not** port across machines: a WSL2/i7-12700H A/B of the plan cache predicted `cpu` −26%; the T470 showed no resolvable difference at that size. Prefer `instructions`/`cycles` over a fixed workload with SMT off (the v3.8.1 case study in `docs/TECHNICAL.md` is the model) — a ±60% swing in instructions is real whatever the clock did; a ±16% swing in `cpu` usually is not. Run-to-run spread on `cpu` reaches 13–18% at 32768/65536.
